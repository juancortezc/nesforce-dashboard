...
export default function SegmentComparisonCard({ month = 'all', year = 'all', region = 'all', group = 'all', position = 'all', route = 'all', kpi = 'all' }: any) {
  const { data, error, isLoading } = useSWR(`/api/results-by-segment?month=${month}&year=${year}&region=${region}&group=${group}&position=${position}&route=${route}&kpi=${kpi}`, fetcher);
